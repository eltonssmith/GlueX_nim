...
the the per-paddle rates. The time resolution must be sufficient to determine the electron RF bunch that determined the event
(see Section\,\ref{sec:ebeam}). It provides a timing signal that is relatively independent of particle type and trajectory (because of its proximity to the target), can provide particle identification via $dE/dx$, and can be used in the Level 1 trigger. Details of the design, construction and performance of the ST system can be found in 
