...
Time-of-Flight (TOF) detector. Both detectors provide timing information but also particle identification based on
energy loss ($dE/dx$) for the former and flight-time for the latter.
...
The ST consists of 30 scintillator paddles arranged in a cylinder of radius 78~mm for
...
selected. EJ-200 has a decay time of 2.1~ns with a bulk attenuation length
of 380~cm. Silicon photomultiplier detectors are used as light
sensors, and these are not affected by the magnetic field produced by the solenoid. The SiPMs were placed
...
The on-board electronics provides two signals per paddle, one delivered to a JLab FADCs, the other to a 5$\times$~amplifier that is sent to a discriminator and then to a JLab F1 TDC. The performance of the ST is discussed in 
...
The TOF detector is a wall of scintillators located about 5.5~m downstream from the target and covers 
...
free of any detector material requiring four shorter, single-PMT paddle detectors with a length of 120~cm around the beam hole
...
The main purpose of the detector is to provide a fast timing signal for charged particles passing through the detector thereby providing information for particle identification.

The scintillator paddles are read out using 
photomultiplier tubes from Hamamatsu~\footnote{Hamamatsu Photonics, https://www.hamamatsu.com/us/en/index.html.}. Full-length paddles
have a PMT at both ends, while the short paddles have a single PMT
at the outer end of the detector. These tubes of type H10534 have 10-stages and are complete assemblies with high voltage base, casing and $\mu$-metal shielding. Due to the significant stray field from the spectrometer solenoid magnet, additional external
...
The sparcification threshold for the fADC250 is set to 120 (160) counts for the ST (TOF), with the nominal pedestal set at 100 counts. The HV of the TOF PMTs are adjusted to generate the amplitude of the signal from a minimum ionizing particle of at least 400 ADC counts above baseline . The data from the fADC250 is provided by the FPGA algorithm and consists
...
The ST can also be used to identify particles using $dE/dx$. Fig.~\ref{fig:ST_dEdx_vs_p} shows $dE/dx$ vs. momentum
for charged particles matched to the Start Counter. Protons can be
separated from pions up to 0.9 GeV/$c$.
...
high certainty. However, at a $\beta$ of 0.98 the probability of the track being a kaon is less than 50\% mainly because the abundance of pions is close to an order of magnitude larger than kaons. The protons, on the other hand, are very well
...
The ST detector, which has full angle acceptance, identifies the RF beam bucket of the beam photon that initiated the event. The TOF detector in the forward region of the GlueX spectrometer provides high resolution timing information which is important for particle identification and contributes to the identification of the RF beam bucket.
