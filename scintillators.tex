...
target referred to as Start Counter (ST) and 2-plane hodoscope detector system in the forward direction referred to as
...
The ST detector, shown in Fig.~\ref{fig:st-overview-drawing},
...
the per-paddle rates. The time resolution must be sufficient resolve the RF-beam structure in order to identify the electron beam-bunch from which the event originated
...
The ST consist of 30 scintillator paddles arranged in a cylinder of radius 78~mm for
...
spectometer solenoid. The SiPMs were placed
at the upstream end of each scintillator element with a 250~$\mu$m air gap.
Each paddle is read out with an array of four 
of these SiPMs (Hamamatsu S109031-050P multi-pixel photon counters), whose signals are summed. 
The on-board electronics provides two signals per paddle, one delivered to a JLab Flash ADC (FADC), the other to a 5$\times$~amplifier that is sent to a discriminator and then to a JLab F1 TDC. The performance of the ST is discussed in 
...
free of any detector material giving rise to four shorter, single-PMT paddle detectors with a length of 120~cm around the beam hole
...
The main purpose of the detector is to provide a fast timing for charged particles passing through the detector thereby providing information for particle identification.
...
a passive splitter into two equal-amplitude signals. One signal is directly connected to a flash ADC250
...
The sparcification threshold for the fADC250 is set to 120 (160) counts for the ST (TOF), with the nominal pedestal set at 100. The HV of the TOF PMTs are set to produce at signal of at leaset 400 ADC counts in amplitude above base line for a minimum ionizing particle. The data from the fADC250 is provided by the FPGA algorithm and consists
...
high certainty. However, at a $\beta$ of 0.98 the probability of the track being a kaon is less than 50\% mainly due to the fact
...
By a track momentum of 4~GeV/c, this has changed and represents the limit at which the TOF can identify protons with high confidence. Again the separation between the large peak containing pions, kaons and positrons is separated from the proton
