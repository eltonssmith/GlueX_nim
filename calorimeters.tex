...
% (mrs: I thought the ring buffer in the FADC was several microseconds long -- what is the 100 sample segmenation that is referred to in the next sentence? ES: This is the window that is used to search for pulses in the readout; the microsecond window is for the trigger.)
...
\includegraphics[width=0.9\textwidth]{figures/data_RMetaBCAL_EnSymmMean_sim_RMetaBCAL_EnSymmMean_sigma.png} 
\caption{\label{fig:bcal:eta_resolution}
The energy resolution of single photons calculated from the $\eta$ mass 
distribution under the assumption that only the energy resolution contributes to its width. 
(Color online)
 }
\end{figure}    
