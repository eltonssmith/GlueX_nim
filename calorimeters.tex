...
To study the $\eta$ mass resolution, we selected events using kinematic fits to $\gamma p \rightarrow p \pi^+ \pi^- \gamma \gamma$, with $\eta\rightarrow \gamma\gamma$.  
The charged pion tracks were used to determine the event vertex needed to accurately reconstruct the two-photon invariant mass.
This reaction provides a fairly clean sample of $\eta$'s where photons are recorded either both in the BCAL or both in the FCAL. 
...
Monte Carlo simulation of $\gamma p \rightarrow p \pi^+ \pi^- \eta$ events, with the kinematics chosen to approximate the experimental distributions, were used to tune the MC resolution to match the data. 
The single-photon resolutions are shown in Fig.\,\ref{fig:bcal:eta_resolution}\,a) for the BCAL and Fig.\,\ref{fig:bcal:eta_resolution}\,b) for the FCAL as a function of the photon energy, both for data and simulation. 
If we average the response over the $\pi^0$ and $\eta$ spectra we find $\sigma_E/E$=5.2\%/$\sqrt{E(\rm{GeV})} \oplus$ 3.6\%. The large constant term is artificially large because of the averaging over angles; at a single angle in the forward direction, the constant term is less than 1.7\%.
...
\includegraphics[width=0.45\textwidth]{figures/data_RMetaBCAL_EnSymmMean_sim_RMetaBCAL_EnSymmMean_sigma.pdf} \includegraphics[width=0.45\textwidth]{figures/data_RMetaFCAL_EnSymmMean_sim_E5p0_3p0_RMetaFCAL_EnSymmMean_sigma.pdf}
\caption{\label{fig:bcal:eta_resolution} 
The energy resolution of single photons in the a) BCAL and b) FCAL calculated from the $\eta$ mass distribution under the assumption that only the energy resolution contributes to its width.  Black circles are data and red squares are simulation.
(Color online)
 }
\end{figure}    
