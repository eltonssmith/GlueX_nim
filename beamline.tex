...
The electron beam position at the radiator is monitored and controlled using beam position monitors (5C11 and 5C11B) which are located at the end of the accelerator tunnel just upstream of the Tagger Hall (see \ref{fig:beam:Draw_beamline}.)
...
The convergence properties of the electron beam are measured by scanning vertical and horizontal wires 
through the beam and measuring the induced current on the wire (i.e. the scattered beam intensity) as a function of position; the wire scanners are referred to as ``harps."
...
radiator, in order to produce a single dominant coherent peak\footnote{Defined as 0.6 GeV below the coherent edge (nominally 9 GeV). The coherent edge varied somewhat depending on the primary incident electron beam energy.}  with the desired energy and polarization
...
atomic number of carbon ($Z=6$), which results in minimal screening of 
...
To be useful as a radiator, the cross-sectional area of a diamond target also must be large enough in the plane orthogonal to the incident beam so as to completely contain the electron beam in order that the beam does not overlap with the material of
the target holder. Translated to the beam spot dimensions from Table\,\ref{tab:elecprop}, \gx{}
...
The Tagger Hodoscope (TAGH) consists of 218 scintillator counters distributed over a length of 9.25~m and mounted just behind the focal plane of the tagger magnet.
...
\subsection{Pair Ppectrometer  \label{sec:ps}}
...
The TAC has near 100\% efficiency and is used to empirically
